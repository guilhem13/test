...
from app.user.repositories.user_repository import AbstractUserRepository
...
class UserSqlAlchemyRepository(AbstractUserRepository):
    ...
    def add(cls, user: User) -> User:
        try:
            models = UserModel(last_name=user.last_name, first_name=user.first_name, phone=user.phone, address=user.address, email=user.email, couple=user.couple, enfants=user.enfants, allergies=user.allergies, regime=user.regime, commentaires=user.commentaires, gphoto=user.gphoto)
            session.add(models)
            session.commit()
            session.flush()
    ...
    @classmethod
    def checkEmailMustUnique(cls, email: str ) -> bool:
        try:
            query = session.query(UserModel).filter(UserModel.email == email)
